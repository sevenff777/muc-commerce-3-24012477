...
def load_dashboard_data(base_dir: Path, selected_category: str = "全部") -> dict:
    ...
    metrics = [
        ...
        {"label": "总体流失率", "value": f"{float(metric_map['流失率']):.1%}", "note": "占比"},
    ...
    highest_risk_stage = highest_risk_row["TenureGroup"]
    highest_risk_rate = highest_risk_row["流失率"]
    insight = (
        f"生命周期阶段“{highest_risk_stage}”的流失率最高，为{highest_risk_rate:.1%}，"
        "说明这类用户是当前最需要重点关注的风险群体。"
    )
